function u_new=MPCcompute_linear_inputCons_outputSoftCons_maxAndmin_kalman(R,Y,D,U,Ad,Bd,Bd_d,Css,u_min,u_max,u_delta_min,u_delta_max,z_min,z_max,N,Q_cof,u_delta_cof,eta_cof1,eta_cof2)
%%%%需要模型信息，A,B,C,D...
num_x=4;
num_u=2;
num_d=2;
...
Gam=zeros(N*num_y,N*num_u+2*N*num_eta);%[H]*[Uk,etak] ->>> y
...
Z_min=z_min*ones(num_z_cons,1)-Phi*Y-Phi_d*D;
Z_max_fake=5000*ones(num_z_cons,1);
Z_max=z_max*ones(num_z_cons,1)-Phi*Y-Phi_d*D;
Z_min_fake=zeros(num_z_cons,1);

U_min=[u_min*ones(N*num_u,1);zeros(2*N*num_eta,1)];
U_max=[u_max*ones(N*num_u,1);5000*ones(2*N*num_eta,1)];%eta上限给个很大的值就可以
